...
public class LobbyManager : MonoBehaviour
{
    ...
    버그 2개 발견.
    1. 비로그인 게임시작 -> 로비로 나온 후 로그인 -> 로그인이 안됨 -> 재시작시 로그인 state가 true로 바뀌는 현상 발생.
    2. 엔딩해금 직후 세이브한 파일을 로드시 엔딩 진행 코루틴에서 씬로드매니저가 삭제됨. (같은 로직 사용하는 인게임 -> 로비이동시 매니저 삭제 로직은 정상작동)

    내 생각. 둘다 해당 로직에서 버그가 발생. 뭔가 씬로드 매니저에 이상이 생겨 로그인도 안돼고 엔딩씬 전환이 안돼는 것 같음.
    */
}
